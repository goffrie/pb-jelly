...
class CodeWriter(object):
    ...
    def gen_msg(
        ...
        with block(self, "pub struct " + escaped_name):
            for idx, field in enumerate(msg_type.field):
                ...
                if typ.oneof:
                    ...
                    self.write("pub %s: %s," % (escape_name(field.name), typ.storage_type()))
            ...
            for oneof in oneof_decls:
                if oneof_nullable(oneof):
                    ...
                    self.write(
                        "pub %s: %s," % (escape_name(oneof.name), oneof_msg_name(name, oneof))
                    )
            ...
            with block(self, "fn default() -> Self"):
                with block(self, escaped_name):
                    for field in msg_type.field:
                        typ = self.rust_type(msg_type, field)
                        if not typ.oneof:
                            self.write("%s: %s," % (escape_name(field.name), typ.default(name)))
                    for oneof in oneof_decls:
                        oneof_field = oneof_fields[oneof.name][0]
                        typ = self.rust_type(msg_type, oneof_field)
                        self.write("%s: %s," % (escape_name(oneof.name), typ.default(name)))
                    if preserve_unrecognized:
        ...
        with block(self, "lazy_static!"):
            self.write(
                "pub static ref %s_default: %s = %s::default();" % (name, escaped_name, escaped_name)
            )
        ...
        with block(self, "impl ::pb_jelly::Message for " + escaped_name):
            ...
            with block(
                ...
                for field in sorted(msg_type.field, key=lambda f: f.number):
                    ...
                    if typ.should_serialize_packed():
                        with block(self, "if !self.%s.is_empty()" % escape_name(field.name)):
                            self.write("let mut size = 0;")
                            with field_iter(self, "val", name, msg_type, field):
                                self.write(
                        ...
                        for field in msg_type.field:
                            ...
                            with block(self, "%s =>" % field.number):
                                if typ.can_be_packed():
                                    with block(self, "match typ"):
                                        with block(
                                            ...
                                            with block(
                                                ...
                                                self.write(
                                                    "self.%s.push(val);" % escape_name(field.name)
                                                )
                                        with block(self, "_ =>"):
                                            self.write(
                                    ...
                                    if typ.is_repeated():
                                        self.write("self.%s.push(val);" % escape_name(field.name))
                                    else:
                                        field_val = (
                        ...
                        with block(self, '"%s" =>' % field.name):
                            if typ.oneof:
                                if len(
                                    ...
                                    with block(self, "match self.%s" % escape_name(typ.oneof.name)):
                                        self.write(
        ...
        for crate, deps in self.deps_map.items():
            ...
            def mod_tree_dfs(
                ...
                content = "\n".join(
                    ...
                    + ["pub mod %s;" % escape_name(mod) for mod in sorted(sub_mod_tree.keys())]
                    + [""]
                )
